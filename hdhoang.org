...
* Firefox
  ...
  #+BEGIN_SRC sh :tangle ~/.bash_login
    function videos() {
        wget -O- \
            ...
            | grep -vE 'reader/api|rapidgator|js|avatar|identico|png|jpg|nippon.com|escapistmag|muspy' \
  ...
  #+BEGIN_SRC vimrc
    ...
    for (var c of string) {
        commands.execute("tabopen "
        + "http://hanviet.org/hv_timchu_ndv.php?unichar=" + c.codePointAt(0))
    }
    EOS
  #+END_SRC
