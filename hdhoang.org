...
** IRC bot
   ...
   It didn't work very well with Vietnamese characters, so it was retired. Here's a replacement in rust and python. Its name is
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     ...
     def list_<<botname>>s(c, e):
         for <<botname>> in filter(lambda n: n.startswith(NAME), e.arguments[-1].split(' ')):
             if <<botname>> not in <<botname>>s:
                 <<botname>>s.append(<<botname>>)
         if c.get_nickname() not in <<botname>>s:
             c.reconnect()
         <<botname>>s.sort()
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def <<botname>>_joins(c, e):
         if e.source.nick not in <<botname>>s:
             <<botname>>s.append(e.source.nick)
             <<botname>>s.sort()
     <<botname>>.on_join = lambda c, e: e.source.startswith(NAME) and <<botname>>_joins(c, e)
   #+END_SRC
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def title(text):
         """
         >>> title('http://www.vietnamplus.vn/tu-14-dong-loat-tang-phi-cao-toc-ha-noihai-phong-quoc-lo-5/378664.vnp')
         'Từ 1/4: Đồng loạt tăng phí cao tốc Hà Nội-Hải Phòng, Quốc lộ 5  | Vietnam+ (VietnamPlus)'
         >>> title('https://<<domain>>/<<botname>>.html https://<<domain>>/<<botname>>.html')
         'IRC bot / IRC bot'
         >>> title('http://www.nytimes.com/2016/01/26/business/marvin-minsky-pioneer-in-artificial-intelligence-dies-at-88.html')
         'Marvin Minsky, Pioneer in Artificial Intelligence, Dies at 88 - The New York Times'
         >>> title('http://www.baomoi.com/bao-nhieu-tan-bot-trung-quoc-da-duoc-nhap-ve-lam-tra-o-long-tea-plus/c/18486151.epi')
         'Bao nhiêu tấn bột Trung Quốc đã được nhập về làm trà Ô long TEA Plus? - GĐ&XH;'
         >>> title('http://news.zing.vn/chi-tiet-ban-do-cam-duong-dip-29-o-ha-noi-post574142.html')
         'Chi tiết bản đồ cấm đường dịp 2/9 ở Hà Nội - Thời sự - Zing.vn'
         """
         titles = []
         urls = filter(lambda w: w.startswith('http'), text.split())
         for u in urls:
             if any(d in u for d in [<<uninteresting>>]):
                    continue
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def wolframalpha(text):
         ...
         r = urlopen('http://api.wolframalpha.com/v2/query?format=plaintext&appid=<<wolframalpha_key>>&input=' + quote(text))
         tree = ET.parse(r)
         reply = ''
         for n in tree.iter():
             if n.tag == 'pod':
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def google(text):
         ...
         data = json.loads(r.read().decode('utf-8'))['responseData']
         r.close()
         if not data['results']:
             return '0 result'
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def translate(text):
         """
         >>> translate('la-en ad astra per aspera')
         'la-en: to the stars through rough'
         >>> translate("vi you think you're good?")
         'en-vi: ngươi nghĩ ngươi giỏi không?'
         >>> translate('en mày nghĩ mày ngon?')
         ...
         if not text:
             return 'Missing text'
         r = urlopen('https://translate.yandex.net/api/v1.5/tr.json/translate?key=<<yandex_key>>&text={}&lang={}'.format(quote(text), lang))
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def post_source():
         ...
         conn.request('POST', '/', 'read:1=3&name:1=<<botname>>.py&f:1=' + quote(open(__file__).read()))
         return conn.getresponse().read().decode('utf-8').strip()
   #+END_SRC
   ...
   Don't do this, just use python3

   #+BEGIN_SRC python :tangle /dev/null
     import sys
     reload(sys)
...
*** IPsec
    #+BEGIN_SRC conf :tangle /sudo:10.117.14.185:/etc/ipsec.conf
      ca letsencrypt
          auto=add
          cacert=/etc/letsencrypt/live/zahe.me/chain.pem

      conn kub
          auto=add
          left=<<prefix>>185
          leftid=kub.<<domain>>
          leftcert=/etc/letsencrypt/live/zahe.me/cert.pem
