...
* Firefox
  ...
  #+BEGIN_SRC vimrc
    command k -js -nargs=* <<EOS
      if (args.length == 0) { args = [dactyl.clipboardRead("global")]; }
      for(var i=0;i<args.length;i++) {
        arg = args[i];
        for (var c=0;c<arg.length;c++) {
          commands.execute("tabopen " + "http://hanviet.org/hv_timchu_ndv.php?unichar=" + arg.codePointAt(c))
    }}
EOS
  #+END_SRC
* Windows
