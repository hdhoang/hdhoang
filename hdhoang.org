...
** IRC bot
   ...
   There are several things it could improve on:
    - [ ] Handle gzip responses (news.zing.vn and baomoi.com): These
    ...
    - [ ] NYtimes access: The cookie needs to be periodically rebaked.
   ...
   #+BEGIN_SRC rust :tangle ~/Public/luser.rs :post rustfmt_luser
     extern crate regex;
     extern crate irc;
   ...
   #+BEGIN_SRC rust :tangle ~/Public/luser.rs
     ...
     fn main() {
         let freenode = IrcServer::from_config(Config {
                            ...
                            alt_nicks: Some((..10).map(|n| format!("{}-{}", NAME, n)).collect()),
   ...
   #+END_SRC
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def <<botname>>_quits(c, e):
         <<botname>>s.remove(e.source.nick)
     <<botname>>.on_quit = lambda c, e: e.source.startswith(NAME) and <<botname>>_quits(c, e)
   #+END_SRC
   #+BEGIN_SRC rust :tangle ~/Public/luser.rs
                     continue 'messages;
                 }
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     ...
     def on_pubmsg(c, e):
         ...
         if msg == "report!":
             return c.privmsg(e.target, "operated by <<username>> with source code " + post_source())
         if msg.startswith('s/'):
             parts = msg.split('/')
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def wolframalpha(text):
         ...
         with urlopen('http://api.wolframalpha.com/v2/query?format=plaintext&appid=<<wolframalpha_key>>&input=' + quote(text)) as r:
             tree = ET.parse(r)
             reply = ''
             for n in tree.iter():
                 if n.tag == 'pod':
   ...
   #+BEGIN_SRC python :tangle ~/Public/luser.py
     def translate(text):
         ...
         if not text:
             return 'Missing text'
         with urlopen('https://translate.yandex.net/api/v1.5/tr.json/translate?key=<<yandex_key>>&text={}&lang={}'.format(quote(text), lang)) as r:
