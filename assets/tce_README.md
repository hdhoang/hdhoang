...
The font works in all operating systems, but will _currently_ only show color
...
_SVG in Open Type_ is a standard by Adobe and Mozilla for color OpenType
...
_Note: This requires `Bitstream Vera` is installed and will change your
systems default serif, sans-serif and monospace fonts._
...
2. `TwitterColorEmoji-SVGinOT-MacOS-14.0.2.zip` - A hack to replace the `Apple Color Emoji` font by [using the same internal name][12]. Install and accept
...
_Reiterating: Only FireFox supports the SVGinOT color emoji for now. Safari and
Chrome will use the fallback black and white emoji._
...
6. Run install.cmd. _Note: This will take some time._
7. Install both new fonts when requested.
8. Done!

[16]: https://www.python.org/downloads/windows/

_Reiterating: Only FireFox and Edge (legacy) support the SVGinOT color emoji for now. Chrome and Edge (Chromium based) will use the
fallback black and white emoji._
...
- SVGO
- make
- [SCFBuild][13] _(Created for this project!)_
