...
fn main() {
    let freenode = IrcServer::from_config(
        Config {
    ...
    for message in freenode.iter() {
        ...
        if msg.args.get(0) != Some(&CHANNEL.to_owned()) { continue }
        // ignore other bots
        if let Some(ref user) = msg.prefix {
            if user.starts_with(NAME)
                || user.contains("bot") { continue }
        }
        ...
        if let Some(url) = url_regex.captures(line)
            .and_then(|caps| caps.at(0)) {
                match scrape_title(url) {
                    Ok(title) => {
                        freenode.send(
                            Command::PRIVMSG(CHANNEL.to_owned(), format!("TITLE: {}", title)))
                            .unwrap();
                    },
                    Err(e) => println!("{:?}", e)
                }
...
fn scrape_title(url: &str) -> Result<String, hyper::error::Error> {
    ...
    match res.headers.get::<ContentType>() {
        Some(&ContentType(Mime(TopLevel::Text, _, _))) => {
            ...
            Ok(Html::parse_fragment(&html).select(&select_title)
               .next().unwrap().first_child().unwrap()
